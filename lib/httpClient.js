...
function HttpClient(operation, params) {
  ...
  function init() {
    ...
    if (process.env.http_proxy) {
      var proxy_host = process.env.http_proxy;
      proxy_host = proxy_host.startsWith('http://') ? '' : 'http://' + proxy_host;
      originalUrl = urlLib.parse(params.url);
      params.url = proxy_host;
    }
